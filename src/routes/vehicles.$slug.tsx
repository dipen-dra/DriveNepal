...
import { getVehicleBySlug, vehicles } from "@/lib/mock-data";
...
function VehicleDetail() {
  ...
  const suggestions = vehicles.filter((x) => x.id !== v.id && x.type === v.type).slice(0, 3);
    ...
    <>
      ...
      <section className="container-page pb-24">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {suggestions.map((s, i) => <VehicleCard key={s.id} v={s} index={i} />)}
        </div>
      </section>
    </>
  );
